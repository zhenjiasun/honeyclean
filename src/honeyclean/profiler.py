...
class AutomatedDataProfiler:
    ...
    def profile_dataset(
        self, data: Union[pd.DataFrame, str, Path], dataset_name: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if self.config.target_col:
            ...
            profiling_results["target_correlation"] = (
                EnhancedAnalyzer.analyze_target_correlation(
                    df_for_analysis, self.config.target_col
                )
            )
            profiling_results["target_distribution"] = (
                EnhancedAnalyzer.analyze_target_distribution(
                    df_for_analysis, self.config.target_col
                )
            )
            ...
            for target_col in target_cols:
                if (
                    target_col in df_for_analysis.columns
                    and not pd.api.types.is_numeric_dtype(df_for_analysis[target_col])
                ):
                    profiling_results[f"categorical_by_{target_col}"] = (
                        EnhancedAnalyzer.analyze_categorical_by_target(
                            df_for_analysis, target_col
                        )
                    )

        if self.config.id_cols:
            logger.info("Performing ID uniqueness analysis...")
            profiling_results["id_uniqueness"] = EnhancedAnalyzer.check_id_uniqueness(
                df_for_analysis, self.config.id_cols
            )

            # Check composite ID if multiple ID columns
            if len(self.config.id_cols) > 1:
                profiling_results["composite_id_uniqueness"] = (
                    EnhancedAnalyzer.check_composite_id_uniqueness(
                        df_for_analysis, self.config.id_cols
                    )
                )
        ...
        profiling_results["dataset_info"]["numeric_columns"] = inferred_type_counts[
            "numeric"
        ]
        profiling_results["dataset_info"]["categorical_columns"] = inferred_type_counts[
            "categorical"
        ]
        profiling_results["dataset_info"]["datetime_columns"] = inferred_type_counts[
            "datetime"
        ]
        profiling_results["dataset_info"]["text_columns"] = inferred_type_counts[
            "categorical"
        ]  # text is subset of categorical
    ...
    def display_formatted_results(self, profiling_results: Dict[str, Any]) -> str:
        ...
        output.append(
            f"Memory Usage (内存使用): {dataset_info.get('memory_usage_mb', 0):.2f} MB"
        )
        output.append(
            f"Missing Values (缺失值): {dataset_info.get('total_missing', 0):,} ({dataset_info.get('missing_percentage', 0):.2f}%)"
        )
        output.append(
            f"Duplicates (重复行): {dataset_info.get('duplicate_count', 0):,}"
        )
        output.append("")

        # Target correlation analysis
        if "target_correlation" in profiling_results:
            for target_col, correlations in profiling_results[
                "target_correlation"
            ].items():
                output.append(
                    StatisticalFormatter.format_correlation_analysis(
        ...
        if "target_distribution" in profiling_results:
            for target_col, target_stats in profiling_results[
                "target_distribution"
            ].items():
                output.append(
                    StatisticalFormatter.format_target_distribution(
        ...
        if "id_uniqueness" in profiling_results:
            output.append(
                StatisticalFormatter.format_id_uniqueness_check(
    ...
    def _create_enhanced_json_report(
        ...
        for column_name, analysis in profiling_results.get("columns", {}).items():
            ...
            if data_type == "numeric":
                ...
                formatted_stats = StatisticalFormatter.format_categorical_stats(
                    analysis
                )
    ...
    def _analyze_column(self, series: pd.Series, column_name: str) -> Dict[str, Any]:
        ...
        try:
            if suggested_type == "numeric" or pd.api.types.is_numeric_dtype(series):
                ...
                recommendations = (
                    DataCleaningRecommendations.get_numeric_recommendations(analysis)
                )
            # Check datetime BEFORE categorical - datetime columns often have object dtype
            elif (
                suggested_type == "datetime"
                or pd.api.types.is_datetime64_any_dtype(series)
                or suggested_type in ["datetime_iso", "datetime_global", "date_iso"]
            ):
                analysis = StatisticalAnalyzer.analyze_datetime(series)
                analysis["type"] = "datetime"
                recommendations = (
                    DataCleaningRecommendations.get_datetime_recommendations(analysis)
                )
                ...
                recommendations = (
                    DataCleaningRecommendations.get_categorical_recommendations(
                        analysis
                    )
                )
            else:
    ...
    def _generate_reports(
        self, profiling_results: Dict[str, Any], df: pd.DataFrame
    ) -> Dict[str, str]:
        ...
        if self.config.generate_json:
            ...
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(
                    enhanced_results, f, indent=2, default=str, ensure_ascii=False
                )
