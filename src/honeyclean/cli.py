...
@click.option(
    "--config", "-c", default="base.toml", help="Path to configuration TOML file"
)
...
@click.option(
    "--sample", "-s", type=int, default=None, help="Sample size for large datasets"
)
@click.pass_context
def profile(
    ctx,
    ...
    try:
        ...
        if not data_file:
            click.echo(
                "Error: No input file specified. Either provide INPUT_FILE argument or set input_data in config.",
                err=True,
            )
            sys.exit(1)
        ...
        click.echo(
            f"Missing Values: {dataset_info['total_missing']:,} ({dataset_info['missing_percentage']:.1f}%)"
        )
...
def analyze(ctx, input_file: str, columns: tuple, output: Optional[str]):
    ...
    try:
        ...
        formatted_output = profiler.display_formatted_results(
...
@click.option(
    "--id-cols", "-i", multiple=True, help="ID columns for uniqueness checking"
)
@click.option(
    "--sample", "-s", type=int, default=None, help="Sample size for large datasets"
)
