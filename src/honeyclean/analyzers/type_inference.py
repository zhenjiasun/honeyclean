...
class DataTypeInference:
    ...
    def infer_column_type(cls, series: pd.Series) -> Dict[str, Any]:
        ...
        for pattern_name, pattern in cls.PATTERNS.items():
            ...
            pattern_scores[pattern_name] = (
                matches / len(sample) if len(sample) > 0 else 0
            )
        ...
        suggested_type = cls._determine_suggested_type(
            pandas_type, best_pattern, best_pattern_score
        )
    ...
    @classmethod
    def _determine_suggested_type(
        cls, pandas_type: str, best_pattern: str, confidence: float
    ) -> str:
