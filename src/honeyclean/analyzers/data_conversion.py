...
class DataTypeConverter:
        ...
        try:
            ...
            if success_rate >= 0.9 and valid_count > 0:
                ...
                conversion_report["partially_convertible_float_columns"][column] = {
                    ...
                    "invalid_examples": (
                        data[column][converted_series.isna()].head(3).tolist()
                        if any(converted_series.isna())
                        else []
                    ),
                }
            else:
    ...
    @staticmethod
    def _process_datetime_conversion(
        data: pd.DataFrame,
        converted_df: pd.DataFrame,
        column: str,
        conversion_report: Dict[str, Any],
            ...
            try:
                if fmt is None:
                    ...
                    converted_series = pd.to_datetime(
                        data[column], errors="raise", infer_datetime_format=True
                    )
                else:
                    converted_series = pd.to_datetime(
                        data[column], format=fmt, errors="raise"
                    )
        ...
        try:
            converted_series = pd.to_datetime(
                data[column], errors="coerce", infer_datetime_format=True
            )
    ...
    def _log_conversion_results(conversion_report: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"Fully convertible float columns: {len(conversion_report['convertible_float_columns'])}"
        )
        logger.info(
            f"Partially convertible float columns: {len(conversion_report['partially_convertible_float_columns'])}"
        )
        logger.info(
            f"Unconvertible to float: {len(conversion_report['unconvertible_float_columns'])}"
        )
        logger.info(
            f"Already float columns: {len(conversion_report['already_float_columns'])}"
        )

        logger.info("=== DateTime Conversion Results ===")
        logger.info(
            f"Fully convertible datetime columns: {len(conversion_report['convertible_datetime_columns'])}"
        )
        logger.info(
            f"Partially convertible datetime columns: {len(conversion_report['partially_convertible_datetime_columns'])}"
        )
        logger.info(
            f"Unconvertible to datetime: {len(conversion_report['unconvertible_datetime_columns'])}"
        )
        logger.info(
            f"Already datetime columns: {len(conversion_report['already_datetime_columns'])}"
        )
    ...
    def analyze_conversion_results(conversion_report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        float_partially_convertible = len(
            conversion_report["partially_convertible_float_columns"]
        )
        ...
        datetime_partially_convertible = len(
            conversion_report["partially_convertible_datetime_columns"]
        )
        ...
        analysis = {
            ...
            "float_analysis": {
                ...
                "success_rate": (
                    (float_convertible + float_partially_convertible) / total_columns
                    if total_columns > 0
                    else 0
                ),
            },
            "datetime_analysis": {
                ...
                "success_rate": (
                    (datetime_convertible + datetime_partially_convertible)
                    / total_columns
                    if total_columns > 0
                    else 0
                ),
        ...
        if float_convertible > 0:
            analysis["recommendations"].append(
                f"Successfully converted {float_convertible} columns to numeric for better analysis"
            )

        if float_partially_convertible > 0:
            analysis["recommendations"].append(
                f"Review {float_partially_convertible} partially convertible numeric columns for data quality issues"
            )

        if datetime_convertible > 0:
            analysis["recommendations"].append(
                f"Successfully converted {datetime_convertible} columns to datetime for time-series analysis"
            )

        if datetime_partially_convertible > 0:
            analysis["recommendations"].append(
                f"Review {datetime_partially_convertible} partially convertible datetime columns for format inconsistencies"
            )

        total_unconvertible = len(
            conversion_report["unconvertible_float_columns"]
        ) + len(conversion_report["unconvertible_datetime_columns"])
        if total_unconvertible > 0:
            analysis["recommendations"].append(
                f"Keep {total_unconvertible} categorical/text columns as-is for categorical analysis"
            )
    ...
    def get_conversion_summary(conversion_report: Dict[str, Any]) -> str:
        ...
        if conversion_report["convertible_float_columns"]:
            summary_lines.append(
                f"✓ Converted to float: {', '.join(conversion_report['convertible_float_columns'])}"
            )

        if conversion_report["partially_convertible_float_columns"]:
            partial_float = list(
                conversion_report["partially_convertible_float_columns"].keys()
            )
            summary_lines.append(
                f"⚠ Partially converted to float: {', '.join(partial_float)}"
            )

        # DateTime conversions
        if conversion_report["convertible_datetime_columns"]:
            summary_lines.append(
                f"✓ Converted to datetime: {', '.join(conversion_report['convertible_datetime_columns'])}"
            )

        if conversion_report["partially_convertible_datetime_columns"]:
            partial_datetime = list(
                conversion_report["partially_convertible_datetime_columns"].keys()
            )
            summary_lines.append(
                f"⚠ Partially converted to datetime: {', '.join(partial_datetime)}"
            )

        # Already converted
        if conversion_report["already_float_columns"]:
            summary_lines.append(
                f"Already float: {', '.join(conversion_report['already_float_columns'])}"
            )

        if conversion_report["already_datetime_columns"]:
            summary_lines.append(
                f"Already datetime: {', '.join(conversion_report['already_datetime_columns'])}"
            )
