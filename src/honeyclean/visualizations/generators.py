...
class VisualizationGenerator:
    ...
    def create_numeric_plot_for_ppt(
        ...
        try:
            ...
            n, bins, patches = ax1.hist(
                clean_series, bins=30, alpha=0.7, color="steelblue", edgecolor="black"
            )
        ...
        try:
            ...
            # 设置标题和轴标签
            ax2.set_title(
                f"{column_name} - 百分位数分布", fontsize=16, fontweight="bold"
            )
    ...
    def create_categorical_plot_for_ppt(
        self, series: pd.Series, column_name: str
    ) -> io.BytesIO:
        ...
        try:
            ...
            if len(top_categories) > 0:
                ...
                ax1.set_title(
                    f"前{len(top_categories)}个类别计数", fontsize=14, fontweight="bold"
                )
                ...
                for i, (bar, value) in enumerate(zip(bars, top_categories.values)):
                    ax1.text(
                        value + max(top_categories.values) * 0.01,
                        i,
                        str(value),
                        va="center",
                        fontsize=10,
                ...
                ax2.set_title(
                    f"前{len(percentages)}个类别百分比", fontsize=14, fontweight="bold"
                )
                ...
                for i, (bar, value) in enumerate(zip(bars2, percentages.values)):
                    ax2.text(
                        value + max(percentages.values) * 0.01,
                        i,
                        f"{value:.1f}%",
                        va="center",
                        fontsize=10,
            ...
            plt.suptitle(
                f"{column_name} - 分类变量分析", fontsize=16, fontweight="bold"
            )
            plt.tight_layout()
        ...
        try:
            ...
            if len(missing_counts) > 0:
                ...
                for i, (bar, value) in enumerate(zip(bars, missing_counts.values)):
                    ...
                    ax.text(
                        value + max(missing_counts.values) * 0.01,
                        i,
    ...
    def create_enhanced_numeric_plot_for_ppt(
        self, series: pd.Series, column_name: str
    ) -> Tuple[io.BytesIO, io.BytesIO]:
    ...
    def create_correlation_plot_for_ppt(
        ...
        try:
            ...
            if not (
                pd.api.types.is_numeric_dtype(df[feature_col])
            ...
            ax.set_title(
                f"{feature_col} vs {target_col} - 相关性分析",
                fontsize=14,
                fontweight="bold",
            )
    ...
    def create_feature_correlation_heatmap_for_ppt(
        self, df: pd.DataFrame, feature_col: str, top_n: int = 5
    ) -> io.BytesIO:
        ...
        try:
            ...
            correlations = (
                numeric_df.corr()[feature_col].abs().sort_values(ascending=False)
            )
    ...
    def create_enhanced_categorical_plot_for_ppt(
        self, series: pd.Series, column_name: str
    ) -> io.BytesIO:
        ...
        try:
            ...
            ax.set_yticklabels(
                [
                    str(cat)[:30] + "..." if len(str(cat)) > 30 else str(cat)
            ...
            ax.set_title(
                f"{column_name} - 类别分布 (前{len(top_categories)}类)",
                fontsize=14,
                fontweight="bold",
            )
    ...
    def create_categorical_target_analysis_for_ppt(
        self, df: pd.DataFrame, feature_col: str, target_col: str
    ) -> io.BytesIO:
        """
        ...
        try:
            ...
            if df_filtered.empty or not pd.api.types.is_numeric_dtype(
            ...
            # 添加标题和标签
            ax.set_title(
                f"{feature_col} 下的 {target_col} 分布（Top {actual_top_n} 类别）",
                fontsize=18,
                fontweight="bold",
                pad=20,
            )
    ...
    def create_datetime_timeseries_plot_for_ppt(
        self, series: pd.Series, column_name: str
    ) -> io.BytesIO:
        """
        ...
        try:
            ...
            ax1.fill_between(
                range(len(x_values)), y_values, alpha=0.3, color="steelblue"
            )
            ...
            if len(x_values) > 20:
                ...
                ax1.set_xticklabels(
                    [x_values[i] for i in range(0, len(x_values), step)],
                    rotation=45,
                    ha="right",
                    fontsize=9,
                )
            ...
            ax1.set_title(
                f"{column_name} - 时间趋势图 ({title_suffix})",
                fontsize=14,
                fontweight="bold",
            )
            ...
            ax2.set_title(
                f"{column_name} - 时间分布柱状图", fontsize=14, fontweight="bold"
            )
