...
class StatisticalFormatter:
    ...
    def format_numeric_stats(stats: Dict[str, Any]) -> str:
        ...
        basic_stats = [
            ...
            [
                "Missing (缺失值)",
                f"{stats.get('missing_count', 'N/A'):,} ({stats.get('missing_percentage', 0):.2f}%)",
            ],
        ...
        outlier_stats = [
            [
                "Z-Score Outliers (Z分数异常值)",
                f"{stats.get('zscore_outliers', 0):,} ({stats.get('zscore_percentage', 0):.2f}%)",
            ],
            [
                "IQR Outliers (四分位距异常值)",
                f"{stats.get('iqr_outliers', 0):,} ({stats.get('iqr_percentage', 0):.2f}%)",
            ],
            [
                "Modified Z-Score (修正Z分数)",
                f"{stats.get('modified_zscore_outliers', 0):,} ({stats.get('modified_zscore_percentage', 0):.2f}%)",
            ],
            [
                "IQR Bounds (四分位距边界)",
                f"[{stats.get('iqr_lower_bound', 0):.2f}, {stats.get('iqr_upper_bound', 0):.2f}]",
            ],
        ]
        ...
        if stats.get("shapiro_p_value") is not None:
            distribution_tests.append(
                [
                    "Shapiro-Wilk p-value (夏皮罗检验p值)",
                    f"{stats.get('shapiro_p_value', 0):.6f}",
                ]
            )
            distribution_tests.append(
                [
                    "Is Normal (是否正态分布)",
                    "Yes (是)" if stats.get("is_normal", False) else "No (否)",
                ]
            )
        if stats.get("dagostino_p_value") is not None:
            distribution_tests.append(
                [
                    "D'Agostino p-value (达戈斯蒂诺检验p值)",
                    f"{stats.get('dagostino_p_value', 0):.6f}",
                ]
            )
    ...
    def format_categorical_stats(stats: Dict[str, Any]) -> str:
        ...
        basic_stats = [
            ...
            [
                ...
                "Yes (是)" if stats.get("is_high_cardinality", False) else "No (否)",
            ],
        ]
        ...
        result += (
            tabulate(
                basic_stats, headers=["Metric (指标)", "Value (值)"], tablefmt="grid"
            )
            + "\n\n"
        )
        ...
        if top_values:
            ...
            result += (
                tabulate(
                    top_values,
                    headers=["Value (值)", "Count (计数)", "Percentage (百分比)"],
                    tablefmt="grid",
                )
                + "\n\n"
            )
        ...
        if rare_count > 0:
            ...
            rare_display = (
                rare_categories[:10] if len(rare_categories) > 10 else rare_categories
            )
    ...
    @staticmethod
    def format_correlation_analysis(
        correlations: Dict[str, Any], target_col: str
    ) -> str:
        ...
        result = (
            f"🎯 CORRELATION WITH TARGET (与目标变量的相关性): {target_col}\n"
            + "=" * 50
            + "\n\n"
        )
    ...
    @staticmethod
    def format_target_distribution(
        target_stats: Dict[str, Any], target_col: str
    ) -> str:
        """Format target column distribution analysis."""
        result = (
            f"🎯 TARGET DISTRIBUTION (目标变量分布): {target_col}\n" + "=" * 50 + "\n\n"
        )
    ...
    def format_id_uniqueness_check(id_results: Dict[str, Dict[str, Any]]) -> str:
        ...
        for col, stats in id_results.items():
            ...
            status = (
                ...
                else f"❌ {duplicate_count:,} duplicates (重复)"
            )
