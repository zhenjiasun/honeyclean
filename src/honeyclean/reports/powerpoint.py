...
class PowerPointGenerator:
    ...
    def create_presentation(
        self, profiling_results: Dict[str, Any], df_original
    ) -> str:
        ...
        for column_name, column_analysis in profiling_results["columns"].items():
            ...
            self._create_column_slide(
                prs, column_name, column_analysis, df_original, profiling_results
            )
    ...
    def _create_column_slide(
        ...
        if analysis.get("type") == "numeric" and "error" not in analysis:
            try:
                ...
                hist_buffer, percentile_buffer = (
                    self.viz_generator.create_enhanced_numeric_plot_for_ppt(
                        series, column_name
                    )
                )
                ...
                if hist_buffer:
                    slide.shapes.add_picture(
                        hist_buffer, Inches(0.5), Inches(1.2), Inches(5.8), Inches(2.8)
                    )
                ...
                if "target_correlation" in profiling_results and self.config.target_col:
                    target_cols = (
                        ...
                        if isinstance(self.config.target_col, list)
                        else [self.config.target_col]
                    )

                    for target_col in target_cols:
                        if (
                            column_name != target_col
                            and target_col in df_original.columns
                        ):
                            try:
                                corr_buffer = (
                                    self.viz_generator.create_correlation_plot_for_ppt(
                                ...
                                if (
                                    target_col
                                    in profiling_results["target_correlation"]
                                    and column_name
                                    in profiling_results["target_correlation"][
                                        target_col
                                    ]["correlations"]
                                ):
                                    corr_val = profiling_results["target_correlation"][
                                        target_col
                                ...
                                corr_error_box = slide.shapes.add_textbox(
                                    Inches(0.5), Inches(4.2), Inches(5.8), Inches(2.5)
                                )
                ...
                try:
                    heatmap_buffer = (
                        self.viz_generator.create_feature_correlation_heatmap_for_ppt(
                    ...
                    heatmap_error_box = slide.shapes.add_textbox(
                        Inches(6.7), Inches(4.2), Inches(5.8), Inches(2.5)
                    )
                ...
                if target_correlation_text:
                    corr_info_box = slide.shapes.add_textbox(
            ...
            try:
                ...
                plot_buffer = (
                    self.viz_generator.create_enhanced_categorical_plot_for_ppt(
                        series, column_name
                    )
                )
                if plot_buffer:
                    slide.shapes.add_picture(
                        plot_buffer, Inches(0.5), Inches(1.2), Inches(12), Inches(3)
                    )
                    ...
                    for target_col in target_cols:
                        if target_col in df_original.columns:
                            try:
                                ...
                                cat_error_box = slide.shapes.add_textbox(
                                    Inches(0.5), Inches(4.5), Inches(12), Inches(2.2)
                                )
            ...
            try:
                ...
                timeseries_buffer = (
                    self.viz_generator.create_datetime_timeseries_plot_for_ppt(
                        series, column_name
                    )
                )
...
{analysis.get('error', '该变量类型暂不支持详细分析')}"""
    ...
    def _create_recommendations_slide(self, prs: Presentation, results: Dict[str, Any]):
        ...
        if all_recommendations:
            recommendations_text = "整体数据集建议:\n\n" + "\n".join(
                [f"• {rec}" for rec in all_recommendations]
            )
            content.text = recommendations_text
        else:
            content.text = "数据集状况良好，无特殊建议。"

    def _create_target_correlation_slide(
        self, prs: Presentation, results: Dict[str, Any]
    ):
        ...
        for target_col, correlations in results["target_correlation"].items():
            ...
            valid_correlations = {
                k: v
                for k, v in correlations["correlations"].items()
                if not pd.isna(v) and k != target_col
            }

            sorted_corrs = sorted(
                valid_correlations.items(), key=lambda x: abs(x[1]), reverse=True
            )
            ...
            bottom_n = (
                sorted_corrs[-display_count:]
                if len(sorted_corrs) > display_count
                else []
            )
            ...
            left_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(1.0), Inches(6), Inches(6)
            )
            ...
            if bottom_n:
                ...
                for col, corr_val in bottom_n:
                    strength = StatisticalFormatter._interpret_correlation(
                        abs(corr_val)
                    )
                    col_display = truncate_col_name(col)
                ...
                right_content = (
                    f"总特征数少于{display_count*2}个，\n无需显示最弱相关特征"
                )

            right_box = slide.shapes.add_textbox(
                Inches(6.8), Inches(1.0), Inches(6), Inches(6)
            )
            ...
            for frame in [left_frame, right_frame]:
                for paragraph in frame.paragraphs:
                    ...
                    if (
                        f"前{display_count}个" in paragraph.text
                        or f"后{display_count}个" in paragraph.text
                    ):
                        paragraph.font.bold = True
                        paragraph.font.size = Pt(12)

    def _create_target_distribution_slide(
        self, prs: Presentation, results: Dict[str, Any]
    ):
        ...
        for target_col, target_stats in results["target_distribution"].items():
            ...
            if target_stats.get("type") == "categorical":
                ...
                distribution_content += (
                    f"• 中位数: {target_stats.get('median', 0):.2f}\n"
                )
        ...
        content_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(1.2), Inches(12), Inches(6)
        )
    ...
    def _create_id_uniqueness_slide(self, prs: Presentation, results: Dict[str, Any]):
        ...
        for col, stats in id_results.items():
            ...
            status = (
                "✅ 唯一" if duplicate_count == 0 else f"❌ {duplicate_count:,} 重复"
            )
            ...
            if duplicate_values:
                uniqueness_content += (
                    f"  - 重复值示例: {', '.join(map(str, duplicate_values[:5]))}\n\n"
                )
    ...
    def _create_enhanced_recommendations_slide(
        self, prs: Presentation, results: Dict[str, Any]
    ):
        ...
        if "target_correlation" in results:
            recommendations_content += (
                "🎯 目标变量建议 (Target Analysis Recommendations):\n"
            )
        ...
        for col_name, analysis in results.get("columns", {}).items():
            ...
            if missing_pct > 50:
                high_priority_issues.append(
                    f"{col_name}: 缺失值过高 ({missing_pct:.1f}%)"
                )
    ...
    def _create_column_statistics_slide(
        self, prs: Presentation, column_name: str, analysis: Dict[str, Any]
    ):
    ...
    def _create_numeric_stats_layout(self, slide, analysis: Dict[str, Any]):
        ...
        basic_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(1.3), Inches(5.8), Inches(2.2)
        )
        ...
        count_str = (
            f"{count_val:,}" if isinstance(count_val, (int, float)) else str(count_val)
        )
        missing_str = (
            f"{missing_count_val:,}"
            if isinstance(missing_count_val, (int, float))
            else str(missing_count_val)
        )
...
标准差 (Std Dev): {analysis.get('std', 0):.4f}"""
        ...
        dist_box = slide.shapes.add_textbox(
            Inches(6.7), Inches(1.3), Inches(5.8), Inches(2.2)
        )
...
峰度 (Kurtosis): {analysis.get('kurtosis', 0):.4f}"""
        ...
        outlier_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(3.8), Inches(5.8), Inches(1.8)
        )
        ...
        zscore_str = (
            f"{zscore_outliers:,}"
            if isinstance(zscore_outliers, (int, float))
            else str(zscore_outliers)
        )
        iqr_str = (
            f"{iqr_outliers:,}"
            if isinstance(iqr_outliers, (int, float))
            else str(iqr_outliers)
        )
        modified_str = (
            f"{modified_zscore_outliers:,}"
            if isinstance(modified_zscore_outliers, (int, float))
            else str(modified_zscore_outliers)
        )
...
修正Z-Score: {modified_str} ({analysis.get('modified_zscore_percentage', 0):.1f}%)"""
        ...
        if analysis.get("shapiro_p_value") is not None:
            test_box = slide.shapes.add_textbox(
                Inches(6.7), Inches(3.8), Inches(5.8), Inches(1.8)
            )
...
是否正态分布: {'是 (Yes)' if analysis.get('is_normal', False) else '否 (No)'}"""
        ...
        if recommendations:
            rec_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(6.0), Inches(12), Inches(1.3)
            )
    ...
    def _create_categorical_stats_layout(self, slide, analysis: Dict[str, Any]):
        ...
        basic_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(1.3), Inches(5.8), Inches(2.5)
        )
        basic_frame = basic_box.text_frame
        basic_frame.word_wrap = True
        ...
        unique_str = (
            f"{unique_count_val:,}"
            if isinstance(unique_count_val, (int, float))
            else str(unique_count_val)
        )
...
高基数 (High Card): {'是 (Yes)' if analysis.get('is_high_cardinality', False) else '否 (No)'}"""
        ...
        if value_counts:
            values_box = slide.shapes.add_textbox(
                Inches(6.7), Inches(1.3), Inches(5.8), Inches(2.5)
            )
        ...
        if rare_categories:
            rare_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(4.0), Inches(5.8), Inches(1.5)
            )
            ...
            rare_display = (
                rare_categories[:8] if len(rare_categories) > 8 else rare_categories
            )
        ...
        if recommendations:
            rec_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(5.8), Inches(12), Inches(1.3)
            )
            rec_frame = rec_box.text_frame
            rec_frame.word_wrap = True
...
年份跨度 (Year Range): {analysis.get('year_range', 'N/A')} 年"""
        ...
        patterns_box = slide.shapes.add_textbox(
            Inches(6.7), Inches(1.3), Inches(5.8), Inches(2.5)
        )
        ...
        years_str = (
            f"{unique_years_val:,}"
            if isinstance(unique_years_val, (int, float))
            else str(unique_years_val)
        )
        months_str = (
            f"{unique_months_val:,}"
            if isinstance(unique_months_val, (int, float))
            else str(unique_months_val)
        )
        days_str = (
            f"{unique_days_val:,}"
            if isinstance(unique_days_val, (int, float))
            else str(unique_days_val)
        )
...
唯一天数 (Unique Days): {days_str}"""
        ...
        if weekday_dist:
            weekday_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(4.0), Inches(5.8), Inches(1.5)
            )
    ...
    def _create_unknown_stats_layout(self, slide, analysis: Dict[str, Any]):
        ...
        # Basic Info Box
        basic_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(1.3), Inches(12), Inches(2)
        )
        basic_frame = basic_box.text_frame
        basic_frame.word_wrap = True
...
该列的数据类型无法自动识别，建议手动检查数据内容。"""
        ...
        if recommendations:
            rec_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(4.0), Inches(12), Inches(2)
            )
            rec_frame = rec_box.text_frame
            rec_frame.word_wrap = True
    ...
    def _create_data_conversion_slide(self, prs: Presentation, results: Dict[str, Any]):
        ...
        partially_convertible_count = len(
            conversion_report.get("partially_convertible_columns", {})
        )
        ...
        for i, (label, count, emoji, color) in enumerate(stats_boxes):
            ...
            stats_box = slide.shapes.add_textbox(
                Inches(x_pos), Inches(1.3), Inches(box_width), Inches(box_height)
            )
        ...
        success_box = slide.shapes.add_textbox(
            Inches(4), Inches(2.8), Inches(4.5), Inches(1)
        )
        ...
        if conversion_report.get("convertible_columns"):
            conv_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(4.2), Inches(5.8), Inches(2.8)
            )
            ...
            if len(conversion_report["convertible_columns"]) > 8:
                conv_text += (
                    f"... 及其他 {len(conversion_report['convertible_columns']) - 8} 列"
                )
        ...
        if recommendations:
            rec_box = slide.shapes.add_textbox(
                Inches(6.7), Inches(4.2), Inches(5.8), Inches(2.8)
            )
            rec_frame = rec_box.text_frame
            rec_frame.word_wrap = True
